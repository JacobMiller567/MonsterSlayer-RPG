...
if (target != noone) {
	...
	if  (!place_meeting(x + hsp, y +vsp, oWall) or !place_meeting(x, y, oWall)){ //and !place_meeting(x,y,oSolids)) {
	  ...
	  /*
	   if (chase.y - 25 > y or chase.y + 25 < y) // TEST
		{
			player_detected = false;
			show_debug_message("Player Not Detected!!!");
		}
	*/
		...
		else if (player_detected and (chase.x + attack_range <= x or chase.x - attack_range >= x) and !(chase.y - 25 > y or chase.y + 25 < y))
		{
